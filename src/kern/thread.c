...
int thread_spawn(const char * name, void (*start)(void *), void * arg) {
    ...
    // create child thread with helper function
    child = create_thread(name, CURTHR->proc);
 ...
 *
 * Inputs:
 *          child_proc - struct process *, child process
 *          parent_tfr - const struct trap_frame*, parent process trap frame
 *
 * Outputs:
 *          int - if child process fork succeeds, return pid; otherwise, return error code.
 *
 * Side Effects:
 *          None.
 */
int thread_fork_to_user (struct process * child_proc, const struct trap_frame * parent_tfr) {
    ...
    // create child thread
    struct thread* child_thr = create_thread("fork_thread", child_proc);
    child_proc->tid = child_thr->id;
    // attach the process to the child thread
    child_thr->proc = child_proc;

    // set up the thread's context to start at thread_finish_fork
    // child             0x80025a00
    // child->proc       0x80025ad0
    // child->stack_base 0x8002cff0
    // child->proc->mtag 0x8000100000080022
    // parent_tfr        0x80012ef0
    _thread_setup(child_thr, child_thr->stack_base, (void (*)(void))_thread_finish_fork, child_thr, parent_tfr);

    // add child thread to ready list
    int saved_intr_state = intr_disable();
    tlinsert(&ready_list, child_thr);
    intr_restore(saved_intr_state);
...
/**
 * static struct thread * create_thread(const char * name, struct process * proc);
 *
 * helper function based on slide to create thread.
 * basically, it is part of the provided thread_spawn function.
 *
 * Inputs:
 *          name - const char*, thread name.
 *          proc - struct process*, process that the thread is belong to.
 *
 * Outputs:
 *          struct thread* - created thread struct.
 *
 * Side Effects:
 *          None.
 */
struct thread * create_thread(const char * name, struct process * proc) {
